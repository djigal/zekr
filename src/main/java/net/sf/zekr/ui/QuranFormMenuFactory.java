...
public class QuranFormMenuFactory {
   ...
   private MenuItem createMenuItem(int swtStyle, Menu parentMenu, String text, String action, int accelerator, String imageKey,
      ...
      if (imageKey != null && SHOW_MENU_IMAGE) {
         try {
            item.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString(imageKey)));
         } catch (Exception e) {
            e.printStackTrace();
         }
      }
   ...
   public void createOrUpdateRecitationMenu() {
      ...
      if (config.getAudio().getCurrent() != null) {
         ...
         for (AudioData audioData : ret) {
            ...
            if (SHOW_MENU_IMAGE) {
               audioItem.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), audioData.isOffline() ? resource
   ...
   private void setTrans(MenuItem mi) {
      try {
         ...
         mi.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString(getTranslationValidityIcon(td))));
      } catch (ZekrMessageException zme) {
         logger.error(zme);
   ...
   private void changePlayerMenuState(PlayStatus data, String text, String image) {
      ...
      if (SHOW_MENU_IMAGE) {
         playItem.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), image));
      }
