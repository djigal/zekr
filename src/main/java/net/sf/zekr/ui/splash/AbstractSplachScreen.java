...
public abstract class AbstractSplachScreen {
	...
	public AbstractSplachScreen(Display display) {
		...
		try (InputStream is = getClass().getResourceAsStream("/" + resource.getString("image.splashScreen"))) {
			if (is != null) {
				splashImage = new Image(display, is);
			}
		} catch (Exception e) {
			logger.error("Could not load splash screen.", e);
		}
	}
