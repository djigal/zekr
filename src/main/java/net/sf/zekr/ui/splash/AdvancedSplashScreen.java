...
public class AdvancedSplashScreen extends AbstractSplachScreen {
	...
	public AdvancedSplashScreen(Display disp) {
		...
		String[] iconPaths = new String[] {
				resource.getString("icon.form16"),
				resource.getString("icon.form32"),
				resource.getString("icon.form48"),
				resource.getString("icon.form128"),
				resource.getString("icon.form256")
		};

		for (String path : iconPaths) {
			try (InputStream is = getClass().getResourceAsStream("/" + path)) {
				if (is != null) {
					imageList.add(new Image(display, is));
				}
			} catch (Exception e) {
				logger.error("Could not load icon: " + path, e);
			}
		}
		shell.setImages(imageList.toArray(new Image[0]));
