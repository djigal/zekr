...
public class BookmarkUtils {
	...
	public static void addBookmarkItemToMenu(Menu parentMenu, final BookmarkItem bookmarkItem) {
		...
		if (bookmarkItem.isFolder()) {
			...
			menuItem.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString("icon.menu.bookmark.closeFolder")));
			...
			menu.addMenuListener(new MenuAdapter() {
				public void menuShown(MenuEvent e) {
					...
					if (pmi != null) {
						pmi.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString("icon.menu.bookmark.openFolder")));
					}
				}
				...
				public void menuHidden(MenuEvent e) {
					...
					MenuItem pmi = m.getParentItem();
					if (pmi != null) {
						pmi.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString("icon.menu.bookmark.closeFolder")));
					}
				}
			});
			 */
			...
			menuItem.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString("icon.menu.bookmark.item")));
			menuItem.setText(StringUtils.abbreviate(bookmarkItem.getName(), GlobalConfig.MAX_MENU_STRING_LENGTH) + " - "
	...
	public static void addBookmarkItemToTree(TreeItem treeItem, BookmarkItem bookmarkItem) {
		...
		if (bookmarkItem.isFolder()) {
			...
			treeItem.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString("icon.bookmark.closeFolder")));
			...
			treeItem.setImage(ImageUtils.loadFromClasspath(shell.getDisplay(), resource.getString("icon.bookmark.item")));
	...
	private static TreeItem _moveTreeItem(Widget parent, TreeItem treeItem, int index, boolean duplicate) {
		...
		if (bookmarkItem.isFolder()) {
			...
			newTreeItem.setImage(ImageUtils.loadFromClasspath(parent.getDisplay(), resource.getString("icon.bookmark.closeFolder")));
		} else {
			newTreeItem.setText(new String[] { bookmarkItem.getName(),
					CollectionUtils.toString(bookmarkItem.getLocations(), ","), bookmarkItem.getDescription() });
			newTreeItem.setImage(ImageUtils.loadFromClasspath(parent.getDisplay(), resource.getString("icon.bookmark.item")));
		}
